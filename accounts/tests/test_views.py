...
class TestActivationViews(APITestCase):
    ...
    def test_activation_put(self):
        ...
        response = self.client.put(reverse('accounts:activate', kwargs={'token': emailtoken}))
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_activation_put_bad_request(self):
        response = self.client.put(reverse('accounts:activate', kwargs={'token': 'badtoken'}))
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class TestLoginViews(APITestCase):
    ...
    def setUp(self):
        ...
        #Activation:
        emailtoken = mail.outbox[0].body.split('/activate/')[1]
        self.client.put(reverse('accounts:activate', kwargs={'token': emailtoken}))
...
class TestUserAuthViews(APITestCase):
    ...
    def setUp(self):
        ...
        emailtoken = mail.outbox[0].body.split('/activate/')[1]
        self.client.put(reverse('accounts:activate', kwargs={'token': emailtoken}))
        token_response = self.client.post(reverse('accounts:login'), self.user, format='json')
        self.auth_headers = {
            'HTTP_AUTHORIZATION': f"Bearer {token_response.data['token']}",
        }

    def test_user_auth_view_get(self):
        response = self.client.get(reverse('accounts:auth-user'), format='json',**self.auth_headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_auth_view_get_aux(self):
        ...
        response = self.client.get(reverse('accounts:auth-user'), format='json',**auth_headers)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class TestUserDetailViews(APITestCase):
    ...
    def test_user_detail_view_get_queryset(self):
        response = self.client.get(reverse('accounts:user-detail', kwargs={'id': self.userobj.id}), format='json',**self.auth_headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['email'], self.user['email'])
class TestUserListViews(APITestCase):
    ...
    def test_user_list_view_get_queryset(self):
        response = self.client.get(reverse('accounts:users'), format='json',**self.auth_headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
